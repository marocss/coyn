...
export default function Main() {
  ...
  function calculateVariation(coinValue, ystdCloseValue) {
    percentage = ((coinValue - ystdCloseValue) * 100) / ystdCloseValue;
  ...
  useEffect(() => {
    (async () => {
      ...
      const response = await coinAPI.get(`/${ticker}/day-summary/2019/${month}/${ystd}`)
      ...
      const formattedCoinValue = Number(responseCurrent.data.ticker.buy).toFixed(2)
      setCoinValue(formattedCoinValue)

      // calculate variation
      let percentage = calculateVariation(formattedCoinValue, yesterdayCoinValue)
      ...
      if (percentage > 0) {
        const res = await gifAPI.get(`search?api_key=${GIF_API_KEY}q=happy&limit=1&offset=${offset}&rating=G&lang=en`)
        const resultGif = res.data.data[0].images.downsized.url
        setGifURL(resultGif)
      } else {
        const res = await gifAPI.get(`search?api_key=${GIF_API_KEY}q=sad&limit=1&offset=${offset}&rating=G&lang=en`)
        const resultGif = res.data.data[0].images.downsized.url
        setGifURL(resultGif)
      }
